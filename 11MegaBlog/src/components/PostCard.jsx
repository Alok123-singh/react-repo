...
function PostCard({$id, title, featuredImage}) {
  ...
  return (
    <Link to={`/post/${$id}`}>
        <div className='w-auto h-[20rem] bg-gray-100 rounded-xl pr-4 pl-4 pt-4'>
            ...
            <h2
            className='text-xl flex justify-center items-center text-center h-auto font-bold flex-wrap'
            >{title}</h2>
        </div>
    </Link>
  )
